...
SimpleScene = function(program, backgroundColor) {
	...
	this.draw = function() {
		...
		var mvLocation = gl.getUniformLocation(program.glProgram, "modelViewMatrix");
		if(mvLocation == null) {
			window.console.log("Warning: uniform modelViewMatrix not used in shader.");
		} else {
			// our model-view transform:
			// - first apply world transformation to a vertex
			...
			for(var i = 0; i < this.shapes.length; i++) {
				...
				var prLocation = gl.getUniformLocation(program.glProgram, "projectionMatrix");
				if(prLocation == null) {
				...
				this.shapes[i].draw(this.program);
			}
		}
